...
// (id, name): no image bytes cross this endpoint, so the response needs no
// device-secret-in-query concession the way api/drive/photo.js does — a
// plain fetch with a header is fine here.
//
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const files = (json.files || []).map((f) => ({ id: f.id, name: f.name }));

    res.statusCode = 200;
    res.setHeader("Content-Type", "application/json");
    // Short — a photo added to the folder should show up on the wall within
    // a poll cycle, not sit behind a stale CDN-length cache.
