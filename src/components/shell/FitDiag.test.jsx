...
/*
  ...
  reason for a test: the grey band along the bottom of the board survived eight
  fixes, and the reason it could is that nothing said which layer was painting.
  This is the apparatus that finally answered it. A diagnostic that fails
  silently is worse than none, because its output still looks like data — so
  these assert that the apparatus works, not that the board looks right.

  What used to be here as well was cover for the delivery surfaces: an
  always-on build badge and a corner tap gesture. Both are retired (#60), the
  band being fixed, and their suites went with them. The payload they delivered
  did not go anywhere — Settings -> Display diagnostics renders exactly these
  rows — so everything below is unchanged and still the thing worth pinning.

  One test here is a regression guard rather than instrumentation cover: "reads
  delta 0 on the device's signature" is the acceptance bar from #59 and the only
  assertion in this repository that compares the page's own frame against the
  screen instead of against another part of the page. It must not be deleted
  with the rest of the diagnostic apparatus if that is ever retired further.

  jsdom has no layout engine, so every getBoundingClientRect is 0x0 and env()
  resolves to nothing. That rules out asserting on most *values* the readout
  reports — those are the device's to supply. What is assertable here is that
  every labelled row exists, that the probe names every layer, and (with the
  one rule of layout that decides it emulated) that the glass check reads PASS.
*/
